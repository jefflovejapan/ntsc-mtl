...
class ChromaLowpassFilter: CIFilter {
    ...
    init(intensity: Intensity, bandwidthScale: Float, filterType: FilterType) {
        ...
        switch intensity {
        case .full:
            let iFunction = Self.lowpassFilter(cutoff: 1_300_000.0, rate: NTSC.rate * bandwidthScale, filterType: filterType)
            ...
            let qFunction = Self.lowpassFilter(cutoff: 600_000.0, rate: NTSC.rate * bandwidthScale, filterType: filterType)
        ...
        case .light:
            let function = Self.lowpassFilter(cutoff: 2_600_000.0, rate: NTSC.rate * bandwidthScale, filterType: filterType)
